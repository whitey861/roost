// In-memory fake of the subset of @supabase/supabase-js we use in tests.
// Supports .from(table).select/insert/update/upsert/delete with the chained
// filters our code calls (.eq, .in, .is, .maybeSingle, .single, .order,
// .limit). Behaviour only needs to be correct for the queries our runtime
// actually issues, not the full client API.
...
class QueryBuilder {
  ...
  select(cols: string = '*'): this {
    ...
    this.selectColumns = cols === '*' ? '*' : cols.split(',').map((c) => c.trim().split(/\s+/)[0]!).filter((s) => s.length > 0);
    return this;
  }
  ...
  // Make the builder thenable so callers can await without .single/.maybeSingle.
...
export function makeFakeClient(): FakeSupabaseClient {
  return new FakeSupabaseClient(new FakeDb());
}
